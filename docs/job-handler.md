...
A SchedulerSS [job](introduction.md#job) class is used to run a user defined Python function. This job class requires the following arguments:
...
Import the AddJob function:
	...
	from schedulerss.AddJob import AddJob
...
## Modify a Job
A job can be modified in two ways: (1) using the job ID, or (2) using the job name. SchedulerSS and NDScheduler do not require job names to be unique. There can be two or more jobs with the same name. It is recommended that the user ensure each job have a unique name, but this is not enforced in NDScheduler. 

It is likely easiest to modify a job using the Web UI, but it can be done in Python.

Import the UpdateJob fuction

	from schedulerss.UpdateJob import UpdateJob
